...
dp.sh.Highlighter.prototype.Trim = function(str)
	{
		return str.replace(/^\s*(.*?)[\s\n]*$/g, '$1');
	}

// This function returns a portions of the string from pos1 to pos2 inclusive
dp.sh.Highlighter.prototype.Copy = function(string, pos1, pos2) {
    return string.substr(pos1, pos2 - pos1);
}

dp.sh.Highlighter.prototype.Chop = function(str)
{
    return str.replace(/\n*$/, '').replace(/^\n*/, '');
}

dp.sh.Highlighter.prototype.Unindent = function(str)
{
...
dp.sh.Highlighter.prototype.Annotate = function(code) {
	...
	for(var i = 0; i < this.matches.length; i++)
	{
        ...
        annotations[annotations.length] = {text:this.Copy(this.code,pos,match.index), style:'plain'};
        annotations[annotations.length] = {text:match.value, style:match.css};

        pos = match.index + match.length;
	}

    annotations[annotations.length] = {text:this.code.substr(pos), style:'plain'};
...
dp.sh.Highlighter.prototype.Highlight = function(code)
{
	...
	this.code = this.Chop(this.Unindent(code));
    ...
    // annotate the code.
    this.annotations = this.Annotate(this.code);

	// if no matches found, add entire code as plain text
	if(this.annotations.length == 1)
	{
		this.AddBit(this.annotations[0].code, this.annotations[0].style);
		this.SwitchToList();
    ...
    for (var i = 0; i < this.annotations.length; i++) {
        this.AddBit(this.annotations[i].text,this.annotations[i].style);
    }
...
dp.sh.GetBrush = function(language) {
    for (var b in dp.sh.Brushes) {
