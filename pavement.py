...
options(
    ...
    virtualenv=Bunch(
        packages_to_install=['backend/python/ext/pip-0.4.1.tar.gz'],
...
def dojo(options):
    ...
    ext_dir = path("ext")
    ext_dir.mkdir()
    destfile = ext_dir / destfile.basename()
...
@task
def generate_vcs():
    """Generates the JavaScript for the native VCS commands. Uses the information
    directly from the installed version of UVC. The output is displayed on the 
    screen for cutting and pasting into the proper place."""
    from uvc import svn
    for command in svn.get_native_commands():
        output = ""
        desc = command.parser.description
        short_description = desc[:desc.find(".")]
        short_description = short_description.replace("\n", "")
        
        # take each line, wrap it in quotes (with a \n) and concatenate with +
        long_description = "\n".join('        "%s\\n" +' % line for line in command.parser.format_help().split("\n"))
        
        # remove the trailing +
        long_description = long_description[:-2]
        
        aliases = "" if not command.aliases else "\n    aliases: %r," % (command.aliases,)
        
        keychain = ""
        
        # svn copy has crazy rules for prompting
        # WC -> WC  - no prompting
        # URL -> WC - keychain
        # WC -> URL - keychain and message
        # URL -> URL - keychain and message
        if command.__name__ == "SVN_copy":
        ...
        elif command.__name__ == "SVN_commit":
            prompting = """\n    prompting: function(command) {
...
""" % (command.name(), aliases, keychain, prompting, short_description, long_description)
        print output