...
class OmniSync(object):
    ...
    def sync(self, source, destination):
        ...
        try:
            ...
            log.exception("Connection to source failed, exiting...")
            self.exit(1)
        try:
            self.destination_transport.connect(self.destination, self.config)
        except:
            log.exception("Connection to destination failed, exiting...")
            self.exit(1)
    ...
    def file_done(self):
        """Increments the file counter, and can be used by subclasses
        to report on progress further to the user."""
        self.file_counter += 1
