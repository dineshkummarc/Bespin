...
# TODO: Should we merge Project and ProjectView?
# TODO: Why do we have save in both File and in Project?
...
from uvc.main import call_uvc, call_uvc_data

log = logging.getLogger("bespin.filesystem")
...
def get_project(user, owner, project_name, create=False, clean=False):
    ...
    if user != owner:
        if not owner.is_project_shared(project_name, user):
            raise NotAuthorized("%s is not allowed to access %s" %
...
class Revision(object):
    """ A class to wrap up a revision, according to the data stored speced in
...
class Project(object):
    ...
    def save_temp_file(self, destpath, contents=None):
        """Save a temporary version of the given path, that's in parallel with
        the official saved version"""
        if "../" in destpath:
            raise BadValue("Relative directories are not allowed")
    ...
    def list_files(self, path=""):
        ...
        if not d.exists():
            raise FileNotFound("Directory %s in %s does not exist"
                              % (path, self.name))
        ...
        for name in names:
            try:
                ...
                # if it's a symlink, it will raise an exception.
    ...
    def _isotime_to_bespintime(self, isotime):
        localtime = time.strptime(isotime[:19], "%Y-%m-%d %H:%M:%S")
        sign = isotime[20:21]
        offset = time.strptime(isotime[21:], "%H%M")
        offset = (offset.tm_hour * 60 * 60) + (offset.tm_min * 60)
        if sign == "-": offset = -offset
        return str(int(time.mktime(localtime) + offset))

    def get_history(self, path):
        """ Call 'uvc log self.location/$path' and return the results so as to
        fulfill the get_history contract
        See also: https://wiki.mozilla.org/Labs/Bespin/DesignDocs/TimeMachine#Historical_Revision_Information
        """
        log = call_uvc_data(self.location, 'log', [ path ])
        return [ dict(
            source = "vcs",
            id = line.get('changeset', ''),
            date = self._isotime_to_bespintime(line.get('date', '')),
            owner = line.get('user', ''),
            description = line.get('summary', '')
        ) for line in log ]
...
class ProjectView(Project):
    ...
    def get_file(self, path, mode="rw", revision=None):
        ...
        if revision is None:
            ...
            """ Call uvc cat on the given project/path to retrieve the given revision.
            This is roughly equivalent to:
            $ cd $project.location
            $ uvc cat $path -r $revision | $return """
            output = call_uvc(self.location, 'cat', [ path, "-r", revision ])
            return output.file.getvalue()
