...
class TextObj(mobwrite_core.TextObj):
  ...
  def setText(self, newText, handle=None, justLoaded=False):
  ...
  def load(self):
    ...
    if STORAGE_MODE == PERSISTER:
      ...
      self.setText(contents, handle=None, justLoaded=True)
      self.changed = False
    ...
    if STORAGE_MODE == FILE:
      ...
      if os.path.exists(filename):
        try:
          ...
          self.setText(infile.read().decode("utf-8"), handle=None, justLoaded=True)
          infile.close()
          self.changed = False
        ...
        self.setText(None, handle=None, justLoaded=True)
        self.changed = False

    if STORAGE_MODE == BDB:
      # Load the text (if present) from database.
      if texts_db.has_key(self.name):
        self.setText(texts_db[self.name].decode("utf-8"), handle=None, justLoaded=True)
  ...
  def save(self, handle):
    ...
    if STORAGE_MODE == PERSISTER:
      self.persister.save(self.name, self.text, handle)
      self.changed = False
      ...
      try:
        ...
        if action["mode"] == "null":
          if access == Access.ReadOnly:
            ...
            try:
              textobj.setText(None, handle=action["handle"])
            finally:
          ...
          if access == Access.ReadOnly:
            ...
            try:
              if textobj.text != data:
                textobj.setText(data, handle=action["handle"])
